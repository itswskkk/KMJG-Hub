...
func NewRouter(h *Handlers, allowedOrigins []string) http.Handler {
	...
	mux.Handle("PUT /api/v1/projects/{id}/members/{userID}/role", authed(http.HandlerFunc(h.handleUpdateProjectMemberRole)))
	mux.Handle("POST /api/v1/projects/{id}/transfer-ownership", authed(http.HandlerFunc(h.handleTransferProjectOwnership)))
	mux.Handle("POST /api/v1/projects/{id}/leave", authed(http.HandlerFunc(h.handleLeaveProject)))
	mux.Handle("DELETE /api/v1/projects/{id}", authed(http.HandlerFunc(h.handleDeleteProject)))
	mux.Handle("GET /api/v1/projects/deleted", authed(http.HandlerFunc(h.handleListDeletedProjects)))
	mux.Handle("POST /api/v1/projects/{id}/restore", authed(http.HandlerFunc(h.handleRestoreProject)))
